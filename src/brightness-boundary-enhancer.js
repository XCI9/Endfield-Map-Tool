...
const BrightnessBoundaryEnhancer = {
    ...
    _buildVerticalGains(height) {
        ...
        for (let y = 0; y < topHeight; y++) {
            const gain = this.CONFIG_TOP.a * Math.exp(-this.CONFIG_TOP.b * y) + this.CONFIG_TOP.c;
        ...
        for (let y = height - bottomHeight; y < height; y++) {
            ...
            const gain = this.CONFIG_BOT.a * Math.exp(-this.CONFIG_BOT.b * distY) + this.CONFIG_BOT.c;
    ...
    _buildHorizontalGains(width) {
        ...
        for (let x = 0; x < leftWidth; x++) {
            const gain = this.CONFIG_LEFT.a * Math.exp(-this.CONFIG_LEFT.b * x) + this.CONFIG_LEFT.c;
        ...
        for (let x = width - rightWidth; x < width; x++) {
            ...
            const gain = this.CONFIG_RIGHT.a * Math.exp(-this.CONFIG_RIGHT.b * distX) + this.CONFIG_RIGHT.c;
